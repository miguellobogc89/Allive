...
export async function getLiveComments(liveId: string): Promise<LiveCommentModel[]> {
...
export async function createLiveComment(
  ...
  const response = await fetch(
    `${API_URL}/api/lives/${encodeURIComponent(liveId)}/comments`,
    {
      ...
      body: JSON.stringify({
        body,
        ...(identity.type === "guest"
          ? { guestId: identity.id, username: "invitado" }
          : {}),
      }),
    },
  );
  ...
  if (!response.ok) {
    throw new Error(data?.error ?? `No se pudo publicar el comentario (${response.status})`);
  }
